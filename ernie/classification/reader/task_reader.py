...
from ernie.classification.batching import pad_batch_data
...
class XWReader(BaseReader):
    '''
    XW读取方式
    '''

    def _read_tsv(self, input_list, quotechar=None):
        '''
        重写读文件方式2：读list
        list每个元素和header一样
        读tab分割的文件，返回examples=[]，里面每一个元素是一个example
        example: Example(text_a='示例a', text_b='示例b', label='1')
        '''
