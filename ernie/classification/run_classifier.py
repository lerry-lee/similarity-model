...
import ernie.classification.reader.task_reader as task_reader
from ernie.classification.model.ernie import ErnieConfig
from ernie.classification.finetune.classifier import create_model, evaluate, predict
from ernie.classification.optimization import optimization
from ernie.classification.utils.args import print_arguments, check_cuda, prepare_logger
from ernie.classification.utils.init import init_pretraining_params, init_checkpoint
from ernie.classification.utils.cards import get_cards
...
def main(args):
    ...
    if args.do_train:
        ...
        if args.init_checkpoint:
            log.info("使用增量训练，热启动模型为：{}".format(args.init_checkpoint))
            init_checkpoint(
                exe,
            ...
            log.info("使用预训练模型：{}".format(args.init_pretraining_params))
            ...
            try:
                ...
                if steps % args.skip_steps != 0:
                    ...
                    if args.is_classify:
                        ...
                        #在这停顿
                        # log_train.info(
                        #     "epoch:%-3d step:%-5d loss:%f "
                        #     "acc:%0.2f progress:%d/%d speed:%f steps/s" %
                        #     (current_epoch, steps,
                        #      outputs["loss"], outputs["accuracy"], current_example, num_train_examples,
                        #      args.skip_steps / used_time))
                        ce_info.append(
                ...
                save_path = os.path.join(args.checkpoints, args.model_file_name)  # 在这停顿,重写模型保存路径
                fluid.io.save_persistables(exe, save_path, train_program)
                log.info("{}/{} 模型检查点已保存".format(args.checkpoints, args.model_file_name))
...
def evaluate_wrapper(args, reader, exe, test_prog, test_pyreader, graph_vars,
    ...
    for ds in args.dev_set.split(','):
        ...
        log.info(evaluate_info + ', file: {}, epoch: {}, steps: {}'.format(
            ds, epoch, steps))
        # log_dev.info('epoch:%-3d step:%-5d ' % (epoch, steps) + evaluate_info + ' file:{}'.format(
        #     ds))
...
log_dev = logging.getLogger("dev_loss")  # 日志1
log_train = logging.getLogger("train_loss")


# 写文件log配置 #日志2
def prepare_log(logger, debug=False, save_to_file="dev_lossInfo_log"):
    # del logger.handlers[0]
    # 删除paddle创建的handler，否则日志输出2次
    formatter = logging.Formatter(fmt='%(asctime)s : %(message)s')
    date = datetime.datetime.now().strftime('%Y-%m-%d %H-%M')
    if not os.path.isdir("dev_loss_log"):
        os.mkdir("dev_loss_log")
    save_to_file = "dev_loss_log/dev_{}.txt".format(date)
...
# 写文件log配置 #日志2
def prepare_log_train(logger, debug=False, save_to_file="dev_lossInfo_log"):
    # del logger.handlers[0]
    # 删除paddle创建的handler，否则日志输出2次
    formatter = logging.Formatter(fmt='%(asctime)s : %(message)s')
    date = datetime.datetime.now().strftime('%Y-%m-%d %H-%M')
    if not os.path.isdir("train_loss_log"):
        os.mkdir("train_loss_log")
    save_to_file = "train_loss_log/train_{}.txt".format(date)
    file_hdl = logging.FileHandler(save_to_file)
    file_hdl.setFormatter(formatter)
    logger.addHandler(file_hdl)
    logger.setLevel(logging.DEBUG)
